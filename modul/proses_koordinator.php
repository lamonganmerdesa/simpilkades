<?php
    ...
    switch($p){
		...
		case "input":
			...
			if ($jumlah == 0 ) {
			    mysqli_query($koneksi, "INSERT INTO koordinator VALUES ('', '$id_dpt', '$username', '$password', '$koordinator')");
			    
			    $sql	= mysqli_query($koneksi, "SELECT * FROM dpt WHERE id='$id_dpt'");
			    $hasil  = mysqli_fetch_array($sql);
			    $nama   = $hasil['nama'];
			    
			    mysqli_query($koneksi, "INSERT INTO petugas VALUES ('', '$id_dpt', '$nama', '$username', '$password', 'koordinator','1')");
				mysqli_query($koneksi, "UPDATE dpt SET rekom='admin', status = '1' WHERE id='$id_dpt'");
				header('location:../index.php?p=koordinator');						   
				break;
			}
			else {
				$message = 'DATA SUDAH ADA DALAM DATABASE';
				header("location:../index.php?p=koordinator&message=$message");						   
				break;
			}
		case "hapus":
			mysqli_query($koneksi, "DELETE FROM koordinator WHERE id_dpt='$_GET[id_dpt]'");
